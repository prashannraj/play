...
class GeneralSettingController extends Controller {
    ...
    public function general() {
        ...
        $genres = Item::active()
            ->hasVideo()
            ->selectRaw("JSON_UNQUOTE(JSON_EXTRACT(team, '$.genres')) as genres")
            ->whereNotNull('team')
            ->whereRaw("JSON_EXTRACT(team, '$.genres') != ''")
            ->pluck('genres')
            ->flatMap(function ($genres) {
                return explode(',', $genres);
            })
            ->unique()
            ->sortBy(function ($genre) {
                return strtolower(substr(trim($genre), 0, 1)) === 'j' ? 0 : 1;
            })
            ->values();
        $selectedGenre = json_decode(gs('genres'));


        return view('admin.setting.general', compact('pageTitle', 'timezones', 'currentTimezone', 'genres', 'selectedGenre'));
    }
    ...
    public function generalUpdate(Request $request) {
        $request->validate([
            ...
            'socket_appuri'   => ['required', 'regex:/^(ws|wss):\/\/([a-zA-Z0-9\-]+\.)+[a-zA-Z]{2,}(\/[a-zA-Z0-9\-._~:\/\?#\[\]@!$&\'()*+,;=]*)?$/'],
            'genres'          => [gs('genre') ? 'required' : 'nullable', 'array', 'max:5'],
        ]);
        ...
        $general->socket_appuri = $request->socket_appuri;
        $general->genres        = $request->genres ?? [];
        $general->save();
    ...
    public function systemConfigurationSubmit(Request $request) {
        ...
        $general->control_socket    = $request->control_socket ? Status::ENABLE : Status::DISABLE;
        $general->ffmpeg            = $request->ffmpeg ? Status::ENABLE : Status::DISABLE;
        $general->tournament        = $request->tournament ? Status::ENABLE : Status::DISABLE;
        $general->live_tv           = $request->live_tv ? Status::ENABLE : Status::DISABLE;
        $general->genre             = $request->genre ? Status::ENABLE : Status::DISABLE;
        $general->request_item      = $request->request_item ? Status::ENABLE : Status::DISABLE;
        $general->save();
    ...
    public function socketDocumentation() {
        $pageTitle = 'WebSocket Application Setup';
        return view('admin.setting.socket_documentation', compact('pageTitle'));
    }
}
