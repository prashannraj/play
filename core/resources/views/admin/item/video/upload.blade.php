...
@section('panel')
    <form id="uploadForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="resolutions-grid">
            <x-resolution-card resolutionName="three_sixty" resolution="360" qualityName="SD Quality" :videoType="1" />
            <x-resolution-card resolutionName="four_eighty" resolution="480" qualityName="SD Quality" :videoType="1" />
            <x-resolution-card resolutionName="seven_twenty" resolution="720" qualityName="HD Quality" :videoType="1" />
            <x-resolution-card resolutionName="thousand_eighty" resolution="1080" qualityName="Full HD Quality" :videoType="1" />
        </div>
...
@push('script')
    <script>
        // jQuery code for form handling and UI interactions
        (function($) {
            "use strict"
            ...
            @if (gs('ffmpeg'))
                ...
                $('#ffmpegUpload').on('submit', function(e) {
                    ...
                    const uploadId = Date.now(); // unique upload identifier
                    let currentChunk = 0;
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (currentChunk < totalChunks) {
                                    ...
                                    $.ajax({
                                        url: "{{ route('admin.item.chunk.video', $item->id) }}",
                                        type: 'POST',
                                        data: {
                                            _token: '{{ csrf_token() }}',
                                            file_name: fileName,
                                        ...
                                        success: function(response) {
                                            ...
                                            availableVideoResolutions(videoPath, itemId)
                                        },
                                    });
                                }
                            }
                ...
                function availableVideoResolutions(videoPath, itemId) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            multiResolution(resolutions, videoPath, itemId);
                        }
                    });
                }

                function multiResolution(resolutions, videoPath, itemId) {
            ...
            // Handle type-option clicks (switching between file upload and URL)
            $('.type-option').on('click', function() {
                // Get the parent selector container
                ...
                // Update the hidden input with the selected type value
                const typeValue = $(this).data('value');
                $resolutionCard.find('input[type="hidden"]').val(typeValue);
            });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function handleFiles(files, uploader) {
                ...
                // Check file size (limit to 1000MB for this example)
                const maxSize = 1000 * 1024 * 1024; // 1000MB
                if (file.size > maxSize) {
                    showError(uploader, 'File size exceeds 1000MB limit');
                    return;
                }
                ...
                let progressContainer = uploader.querySelector('.progress-load');
                let progressBar = uploader.querySelector('.progress-bar-load');
                ...
                simulateUpload(file, progressBar, uploader, watchVideo);
            }

            // Function to simulate file upload with progress
            function simulateUpload(file, progressBar, uploader, watchVideo) {
                let width = 0;
                ...
                const simulateProgress = setInterval(() => {
                    ...
                    if (width >= 100) {
                        ...
                        uploadComplete(uploader, file, watchVideo);
                    }
            ...
            function uploadComplete(uploader, file, watchVideo = true) {
                ...
                watchVideoText.addEventListener('click', function(e) {
                    ...
                    showVideoModal(videoURL, file.name);
                });
...
@push('style')
    <style type="text/css">
        ...
        .multiple-video-box .spinner-border {
            border: var(--bs-spinner-border-width) solid #5b6e88;
