        ...
        <div class="wishlist-card-wrapper">
            ...
            @if ($party->status == Status::ENABLE)
                <div class="wishlist-card-wrapper__icon">
                    <button class="text--danger basicConfirmationBtn" data-action="{{ route('user.watch.party.disabled', $party->id) }}" data-question="@lang('Are you sure to disabled this party room?')" type="button"><i class="las la-times"></i></button>
                </div>
            @endif
        </div>
    </li>
